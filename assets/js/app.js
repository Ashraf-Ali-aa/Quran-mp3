var Audio = Backbone.Model.extend({
    ...
    default_images: {
         small: player_root + "static/artists/_small.jpg",
        medium: player_root + "static/artists/_medium.jpg",
         large: player_root + "static/artists/_large.jpg"
    },
    initialize: function () {
        if (!this.has("image")) {
            this.set({
    ...
    urlRoot: player_root + "more.php?t=audio&audioid=",
    url: function () {
...
var TrendingAudioCollection = AudioCollection.extend({
    url: function () {
        return player_root + "more.php?t=" + this.path + "&genre=" + this.genre + "&start=" + this.start + "&results=" + this.results
    }
});
var AudioLovedFeedCollection = AudioCollection.extend({
    hasMore: false,
    url: function () {
...
var TimelineLovedCollection = AudioCollection.extend({
    hasMore: false,
    url: function () {
...
var LatestPlaylistCollection = AudioCollection.extend({
    hasMore: false,
    url: function () {
...
var AudioGenreCollection = AudioCollection.extend({
    hasMore: true,
    url: function () {
...
var TOPAudiosCollection = AudioCollection.extend({
    url: function () {
...
var UserLovedCollection = AudioCollection.extend({
    username: null,
    hasMore: true,
    url: function () {
...
var AudioSearchCollection = AudioCollection.extend({
    hasMore: false,
    totalResults: 0,
    url: function () {
...
var AudioGraph = Backbone.Model.extend({
    url: function () {
        return player_root + "api/audio/" + this.get("id") + "/graph"
    },
    parse: function (a) {
        return a.graph
    }
...
var AudioView = Backbone.View.extend({
    showAvatar: true,
    user: null,
    template: Templates.common_audios,
    tagName: "div",
    className: "audio_row a_audio",
    events: {
        "click .audio_view_love": "onLovedClicked",
        "click .audio_view_share": "onShareClicked",
        "click .audio_view_play_button": "onPlayButtonClicked",
        "click .audio_view_queue": "onQueueClicked",
        "click .audio_view_remove": "onRemoveClicked"
    },
    initialize: function (a) {
        _.extend(this, a);
    ...
    render: function () {
        $(this.el).html(this.template({
            audio: this.model.toJSON(),
            user: this.user,
            showAvatar: this.showAvatar
        }));
        var c = this.model.get("id");
        $(this.el).attr("audio_id", c);
        $(this.el).addClass("a_audio_" + c);
    ...
    onPlayButtonClicked: function (b) {
		if ($(b.target).parents(".playing").length != 1) {
			var el = $(this.el).find('.audio_view_queue').addClass("added");
			$(window).trigger({
			    "type" : "qmp3QueueAudio",
			    "audio" : this.model.toJSON()
			});
			var audio_pos = AudioPlayer.List.current.length - 1;
            $(window).trigger({
                type: "qmp3NewAudioList",
    ...
    onLovedClicked: function (b) {
        var a = $(this.el).find(".audio_view_love").addClass("loading");
        $(window).trigger({
            type: (a.hasClass("on")) ? "qmp3UnLoveAudio" : "qmp3LoveAudio",
            audio: this.model.toJSON()
        });
        return false
    },
    ...
    onQueueClicked: function (b) {
		//$('#dropdown-1').hide();
		var audioinfo = this.model.toJSON();
		AUDIO_PRE_ADD = audioinfo.id;
		var a = $(this.el).find(".audio_view_queue");
		//PlaylistMenu.Show(a);
    ...
    onRemoveClicked: function (b) {
		...
		$.ajax({
			...
			data: {audio_id:audioinfo.id, playlist_id:audioinfo.playlist_id, t:"playlist", action:"remove_audio"},
...
var AudioListView = Backbone.View.extend({
    ...
    initialize: function (a) {
        ...
        $("#right").unbind();
        this.audios = new this.collectionClass(a);
        this.audios.bind("add", this.onAdd);
        this.audios.bind("reset", this.onInitialFetch);
        this.audios.bind("fetch", this.onFetch);
        this.render();
        this.itemRows = $("#item_rows");
        this.scrollDiv = $("#item_rows");
        $(this.scrollDiv).unbind();
        this.audios.fetch();
        $(".audio_top_play_all").bind("click", this.onPlayAllClicked);
    },
    ...
    onScrollBottom: function (a) {
        ...
        if (this.audios.hasMore) {
            this.audios.start += this.audios.results;
            this.audios.fetch({
                add: true
            })
        }
...
$(window).bind("qmp3AudioLoved", function (a) {
    $(".audio_view_love_" + a.audio.id).removeClass("loading");
    if (a.success == true) {
        $(".audio_view_love_" + a.audio.id).addClass("on").attr("tooltip", "Loved")
    }
});
$(window).bind("qmp3AudioUnLoved", function (a) {
    $(".audio_view_love_" + a.audio.id).removeClass("loading");
    if (a.success == true) {
        $(".audio_view_love_" + a.audio.id).removeClass("on").attr("tooltip", "Love this audio")
    }
});
$(window).bind("qmp3AudioNewAudio", function (a) {
    jQuery(".a_audio").removeClass("playing");
    jQuery(".a_audio_" + a.audio.id).addClass("playing")
});
    ...
    if (c.href.indexOf("settings") != -1) {
        ...
        switch (c.href) {
            case "settings/profile":
                jQuery(window).trigger({
                    type: "qmp3SettingsProfile"
                });
                break;
            case "settings/social":
                jQuery(window).trigger({
                    type: "qmp3SettingsSocial"
                });
                break;
            case "settings/connections":
                jQuery(window).trigger({
                    type: "qmp3SettingsConnections"
                });
                break;
            case "settings/design":
                jQuery(window).trigger({
                    type: "qmp3SettingsDesign"
                });
                break;
            case "settings/find-friends":
                jQuery(window).trigger({
                    type: "qmp3SettingsFriends"
                });
                break;
            case "settings/account":
                jQuery(window).trigger({
                    type: "qmp3SettingsAccount"
                });
                break;
...
jQuery(window).bind("qmp3HistoryChange", Settings.Change);
jQuery(window).bind("qmp3UserLoggedIn", Settings.LoggedIn);
...
UIinit.Init = function () {
    jQuery(window).bind("qmp3Me", UIinit.Me);
    jQuery(window).trigger({
        type: "qmp3GetMe",
...
UIinit.Me = function (a) {
    jQuery(window).unbind("qmp3Me", UIinit.Me.response);
...
AudioPlayer.Load = function (a) {
    ...
    AudioPlayer.IsLoaded = false;
    jQuery(window).trigger({
        type: "qmp3AudioNewAudio",
        audio: AudioPlayer.List.current[AudioPlayer.QueueNumber],
...
AudioPlayer.Events = {
    ...
    halfAudio: function (a) {
        jQuery(window).trigger({
            type: "qmp3AudioAudioHalf",
            audio: a
        })
    },
    error: function (a) {
    ...
    loadStart: function () {
        jQuery(window).trigger({
            type: "qmp3AudioLoadStart"
        })
    },
    canPlay: function () {
        AudioPlayer.IsLoaded = true;
        jQuery(window).trigger({
            type: "qmp3AudioCanPlay"
        })
    },
    currentAudio: function (d, e, a, b, c) {
        AudioPlayer.QueueNumber = e;
        jQuery(window).trigger({
            type: "qmp3AudioCurrentAudio",
            audio: d,
            queueNumber: e,
            paused: a,
            currentTime: b,
...
AudioPlayer.TimeUpdate = function () {
    if (AudioPlayerHalfAudio == false) {
        if (this.currentTime / this.duration > 0.5) {
            AudioPlayerHalfAudio = true;
            AudioPlayer.Events.halfAudio(AudioPlayer.List.current[AudioPlayer.QueueNumber])
        }
    }
};
...
AudioPlayer.Scrobble = {
    nowplaying: {
        method: player_root + "more.php?t=now_playing&audioid=",
        listener: function (a) {
            var b = a.audio;
    ...
    scrobble: {
        method: player_root + "more.php?t=scrobble.php?audioid=",
        listener: function (a) {
            var b = a.audio;
            if (Utils.HasValue(b.title) == true && Utils.HasValue(b.artist) == true) {
...
jQuery(window).bind("qmp3NewAudioList", AudioPlayer.List.replace);
jQuery(window).bind("qmp3AudioSeek", AudioPlayer.Seek);
jQuery(window).bind("qmp3VolumeChange", AudioPlayer.Volume.change);
...
jQuery(window).bind("qmp3ChangeQueueNumber", AudioPlayer.QueueNumberChange);
jQuery(window).bind("keyBoardLove", AudioPlayer.Love.keyup);
jQuery(window).bind("keyBoardShare", AudioPlayer.Share.keyup);
jQuery(window).bind("qmp3ChangeQueueOrder", AudioPlayer.List.reorder);
jQuery(window).bind("qmp3AudioNewAudio", AudioPlayer.Scrobble.nowplaying.listener);
jQuery(window).bind("qmp3AudioAudioHalf", AudioPlayer.Scrobble.scrobble.listener);
jQuery(window).bind("qmp3AudioLoved", AudioPlayer.List.love);
jQuery(window).bind("qmp3AudioUnLoved", AudioPlayer.List.unLove);
jQuery(window).bind("qmp3QueueAudio", AudioPlayer.List.add);
...
jQuery(window).bind("qmp3NeedHistoryChange", History.NeedChange);
jQuery(window).bind("qmp3UIInit", History.PageLoad);
...
jQuery(window).bind("qmp3HistoryChange", LeftSelect.Change);
jQuery(window).bind("qmp3NewAudioList", LeftSelect.ShowPlaying.determine);
jQuery(window).bind("qmp3AudioStop", LeftSelect.ShowPlaying.clear);
jQuery(window).bind("qmp3LeftSelectBuilt", LeftSelect.Rebuilt);
jQuery(window).bind("qmp3AvatarSet", LeftSelect.RefreshAvatar);
...
jQuery(window).bind("qmp3ShortcutAdd", LeftShortcuts.Add.listener);
(function () {
    ...
    jQuery(window).bind("qmp3UIInit", function (b) {
        if (b.user != null) {
    ...
    jQuery(window).bind("qmp3UserFollow", function (d) {
        ...
        jQuery("#audio_tab_number_following").text(c)
    });
    jQuery(window).bind("qmp3UserUnFollow", function (f) {
        ...
        jQuery("#audio_tab_number_following").text(d)
    });
    jQuery(window).bind("qmp3UserAddPlaylist", function (d) {
        window.LeftPlaylistCollection.add(d.playlist);
        var b = parseInt(jQuery(".playlist_list_count").text());
        var c = b + 1;
        jQuery(".playlist_list_count").text(c);
        jQuery("#audio_tab_number_playlist").text(c)
    });
})();
var UserProfileView = Backbone.View.extend({
    ...
    initialize: function () {
        ...
        jQuery(".audio_tab").removeClass("selected");
        jQuery(".audio_tab_" + this.options.kind).addClass("selected");
        $("#right").unbind();
        Utils.HideSections("#audio_list");
    ...
    render: function () {
        $(this.el).html(this.template(this.model.toJSON()));
        jQuery("#audio_list").removeClass("display_none");
        jQuery(".audio_tab").removeClass("selected");
        jQuery(".audio_tab_" + this.options.kind).addClass("selected");
        section = "user";
        ...
        if (this.options.kind == "audios") {
            jQuery(".audio_tab_audios").addClass("selected");
        ...
        if (this.options.kind == "following") {
            jQuery(".audio_tab_following").addClass("selected");
        ...
        if (this.options.kind == "followers") {
            jQuery(".audio_tab_followers").addClass("selected");
        ...
        if (this.options.kind == "activity") {
            jQuery(".audio_tab_activity").addClass("selected");
        ...
        if (this.options.kind == "feed") {
            jQuery(".audio_tab_feed").addClass("selected");
        ...
        if (this.options.kind == "playlist") {
            jQuery(".audio_tab_playlist").addClass("selected");
    ...
    error: function () {
        jQuery(window).trigger({
            type: "qmp3ShowErrorPage",
            el: this.el
        })
    }
});
var UserLovedView = AudioListView.extend({
...
var UserFeedView = AudioListView.extend({
    collectionClass: AudioLovedFeedCollection,
    section: "feed",
    render: function () {
        Utils.ShowLoading("#item_rows");
...
jQuery(window).bind("qmp3HistoryChange", function (d) {
    switch (d.href) {
        case "":
            break;
        case "/":
...
BottomControls.CurrentAudio = function (a) {
    if (a.paused == true) {
...
jQuery(window).bind("qmp3AudioStop", BottomControls.ShowPlay);
jQuery(window).bind("qmp3AudioPlay", BottomControls.ShowPause);
jQuery(window).bind("qmp3AudioPause", BottomControls.ShowPlay);
jQuery(window).bind("qmp3AudioCurrentAudio", BottomControls.CurrentAudio);
...
BottomDisplay.Init = function (a) {
    ...
    jQuery("#current_audio_love_icon").bind("click", BottomDisplay.Love.click);
    jQuery("#current_audio_share_icon").bind("click", BottomDisplay.Share.click);
...
BottomDisplay.CurrentAudio = function (a) {
    var b = a.audio;
    if (b != undefined) {
        BottomDisplay.WhenPlaying();
        BottomDisplay.Resize();
...
BottomDisplay.Love = {
    show: function (a) {
        if (a.viewer_love != null) {
            jQuery("#current_audio_love_icon").addClass("on");
            jQuery("#current_audio_love_icon").attr("tooltip", "Unlike")
        } else {
            jQuery("#current_audio_love_icon").removeClass("on");
            jQuery("#current_audio_love_icon").attr("tooltip", "Love this audio")
        }
        jQuery("#current_audio_love_icon").addClass("current_audio_love_icon_" + a.id)
    },
    click: function (a) {
    ...
    addOn: function (a) {
        jQuery(".current_audio_love_icon_" + a.audio.id).removeClass("loading");
        if (a.success == true) {
            jQuery(".current_audio_love_icon_" + a.audio.id).addClass("on")
        }
    },
    removeOn: function (a) {
        jQuery(".current_audio_love_icon_" + a.audio.id).removeClass("loading");
        if (a.success == true) {
            jQuery(".current_audio_love_icon_" + a.audio.id).removeClass("on")
        }
    }
};
BottomDisplay.Share = {
...
BottomDisplay.ShowText = function (a) {
    jQuery("#display_audio").html("");
    jQuery("#display_audio").removeAttr("href");
    ...
    jQuery("#display_audio").html(Utils.BlankUndefined(a.title));
    jQuery("#display_audio").attr("href", "/audio/" + a.id);
    var c = "";
    if(a.artists){ 
...
BottomDisplay.Volume = {
    ...
    event: function (a) {
        ...
        qmp3ProfilePlayer.Volume.set(b)
    },
...
jQuery(window).bind("qmp3AudioCreated", BottomDisplay.Init);
jQuery(window).bind("qmp3AudioNewAudio", BottomDisplay.NewAudio);
jQuery(window).bind("qmp3AudioCurrentAudio", BottomDisplay.CurrentAudio);
jQuery(window).bind("qmp3AudioStop", BottomDisplay.Stop);
jQuery(window).bind("resize", BottomDisplay.Resize);
jQuery(window).bind("qmp3AudioCanPlay", BottomDisplay.HideLoading);
jQuery(window).bind("qmp3AudioLoved", BottomDisplay.Love.addOn);
jQuery(window).bind("qmp3AudioUnLoved", BottomDisplay.Love.removeOn);
...
PlaylistContainer.clearAll = function(){
    ...
    AudioPlayer.List.current = []; 
    Storage.Set("queue", AudioPlayer.List.current);
    jQuery(window).trigger({"type" : "qmp3NewPlaylist", "list" : AudioPlayer.List.current});
}
PlaylistContainer.Build = function () {
    ...
    if (PlaylistContainer.List != null) {
        ...
        jQuery("#current_playlist_rows").dragsort({
            ...
            scrollSpeed: 8,
            dragSelectorExclude: ".current_playlist_play_button, .current_playlist_audiotitle, .current_playlist_artist, .current_playlist_love_icon, .current_playlist_link, .current_playlist_remove_icon"
        })
    }
    return true
};
...
PlaylistContainer.LoveIcon = {
    click: function (b) {
        ...
        if (jQuery(this).hasClass("on") == true) {
            jQuery(window).trigger({
                type: "qmp3UnLoveAudio",
                audio: PlaylistContainer.List[a]
            })
        } else {
            jQuery(window).trigger({
                type: "qmp3LoveAudio",
    ...
    addOn: function (a) {
        jQuery(".current_playlist_love_icon_" + a.audio.id).removeClass("loading");
        if (a.success == true) {
            jQuery(".current_playlist_love_icon_" + a.audio.id).addClass("on")
        }
    },
    removeOn: function (a) {
        jQuery(".current_playlist_love_icon_" + a.audio.id).removeClass("loading");
        if (a.success == true) {
            jQuery(".current_playlist_love_icon_" + a.audio.id).removeClass("on")
        }
    }
};
...
jQuery(window).bind("qmp3AudioNewAudio", PlaylistContainer.PlayButton.select);
jQuery(window).bind("qmp3NewPlaylist", PlaylistContainer.Change);
...
jQuery(".current_playlist_audiotitle").live("click", PlaylistContainer.PlayButton.click);
...
jQuery(window).bind("qmp3AudioLoved", PlaylistContainer.LoveIcon.addOn);
jQuery(window).bind("qmp3AudioUnLoved", PlaylistContainer.LoveIcon.removeOn);
...
jQuery(".tooltip, .audio_row_recent_loves_avatars, .audio_page_recent_loves_avatars").live("mouseover", Tooltip.Mouseover);
jQuery(".tooltip").live("mouseout", Tooltip.Mouseout);
jQuery(window).bind("qmp3HistoryChange", Tooltip.Mouseout);
if (typeof (LoveAudio) == "undefined") {
    LoveAudio = {}
}
LoveAudio.Love = {
    method: player_root + "more.php?t=love&action=love&audioid=%id%",
    request: function (c) {
        var d = c.audio;
        if (loggedInUser == null) {
            LoveAudio.Love.trigger(d, false);
            alert('<a href="/sign-in">Login</a> to Love this audio.', true)
        } else {
            var a = {
                client_id: "qmp3_web"
            };
            ...
            var f = LoveAudio.Love.method.replace("%id%", d.id);
            jQuery.ajax({
                url: f,
                type: "POST",
                dataType: "json",
                data: a,
                complete: LoveAudio.Love.response,
                context: {
                    audio: d
                }
            })
    ...
    response: function (a, d) {
        var b = Utils.APIResponse(a, "LoveAudio.Love.response");
        var c = b.json.audio || this.audio;
        if (b.success == true) {
            LoveAudio.Love.trigger(c, true);
            LoveAudio.Love.updateLocalStorage(c)
        } else {
            if (b.json.status_text == "Already loved this audio") {
                LoveAudio.Love.trigger(c, true);
                LoveAudio.Love.updateLocalStorage(c)
            } else {
                LoveAudio.Love.trigger(c, false);
                alert("There was a problem. Please try again.")
            }
        }
    },
...
LoveAudio.UnLove = {
    method: player_root + "more.php?t=love&action=unlove&audioid=%id%",
    request: function (b) {
        var c = b.audio;
        if (loggedInUser == null) {
            LoveAudio.UnLove.trigger(c, false);
            alert('Please <a href="/sign-in">login</a> to un-love audios', true)
        } else {
            var d = LoveAudio.UnLove.method.replace("%id%", c.id);
            var a = Utils.get_cookie("_xsrf");
            jQuery.ajax({
                url: d,
                type: "POST",
                dataType: "json",
                complete: LoveAudio.UnLove.response,
                context: {
                    audio: c
                }
            })
    ...
    response: function (b, d) {
        var c = Utils.APIResponse(b, "LoveAudio.UnLove.response");
        var a = c.json.audio || this.audio;
        if (c.success == true) {
            LoveAudio.UnLove.trigger(c.json.audio, true);
            LoveAudio.UnLove.updateLocalStorage(a)
        } else {
            if (c.json.status_text == "Havent loved this audio") {
                LoveAudio.UnLove.trigger(a, true);
                LoveAudio.UnLove.updateLocalStorage(a)
            } else {
                LoveAudio.UnLove.trigger(this.audio, false);
                alert("There was a problem. Please try again.")
            }
        }
    },
    trigger: function (b, a) {
        jQuery(window).trigger({
            type: "qmp3AudioUnLoved",
            audio: b,
            success: a
        })
    },
    updateLocalStorage: function (d) {
        var f = Storage.Get("sitesAudios");
        if (f != null) {
            for (var c in f) {
                var e = f[c];
                if (e.id == d.id) {
...
jQuery(window).bind("qmp3LoveAudio", LoveAudio.Love.request);
jQuery(window).bind("qmp3UnLoveAudio", LoveAudio.UnLove.request);
...
jQuery(window).bind("qmp3ModalCreated", SignIn.ModalListener);
jQuery(window).bind("qmp3HistoryChange", SignIn.Change);
...
CreateAccount.Show = function () {
    CreateAccount.HasService = null;
    CreateAccount.HasProfileInfo = false;
    CreateAccount.Fireqmp3UserLoggedInEvent = false;
...
CreateAccount.Service = {
    click: function () {
        jQuery(".create_account_oauth").unbind("click", CreateAccount.Service.click);
        var a = jQuery(this).attr("service");
        jQuery(window).bind("qmp3ServiceConnectionAdd", this, CreateAccount.Service.listener);
        window.open("/create-account/" + a)
    },
    listener: function (b) {
        if (b.success == true) {
...
jQuery(window).bind("qmp3ModalCreated", CreateAccount.ModalListener);
jQuery(window).bind("qmp3HistoryChange", CreateAccount.Change);
            ...
            if (b == true) {
                ...
                if(AUDIO_PRE_ADD == "QUEUE"){
                    ...
                    $.ajax({
					    type: "GET",
					    url: player_root + "more.php",
					    data: {audio_id:AUDIO_PRE_ADD, playlist_id:c.playlist_id, t:"playlist", action:"addaudio"},
...
CreatePlaylist.Response = function (b, c) {
	var a = JSON.parse(b.responseText);
	jQuery(window).trigger({
		type: "qmp3UserAddPlaylist",
		playlist: a.playlist
	});
    CreatePlaylist.Button.done(true, b)
};

jQuery(window).bind("qmp3ModalCreated", CreatePlaylist.ModalListener);
...
/*jQuery(window).bind("qmp3ModalCreated", EditPlaylist.ModalListener);*/
...
jQuery(window).bind("qmp3AvatarSet", TopRightDropdown.RefreshAvatar);
jQuery(window).bind("qmp3UIInit", TopRightDropdown.Init);
...
jQuery(window).bind("qmp3UIInit", QueueToPlaylist.Init);
...
PlaylistMenu.Click = function (a) {
    ...
    $.ajax({
        type: "GET",
        url: player_root + "more.php",
        data: {audio_id:AUDIO_PRE_ADD, playlist_id:PLAYLIST_ID, t:"playlist", action:"addaudio"},
        success: function (response) {
            return false;
        }
    });
...
jQuery(window).bind("qmp3UIInit", PlaylistMenu.Init);

if (typeof (BrowseHistory) == "undefined") {
    BrowseHistory = {}
}

BrowseHistory.AudiosList = [];
BrowseHistory.Built = false;
BrowseHistory.CurrentAudios = [];
        ...
        if (BrowseHistory.Built == false) {
            ...
            if (g != null) {
                ...
                for (var d in g) {
                    var f = g[d];
                    if (f.id != null) {
                        BrowseHistory.AudiosList.push(f);
                ...
                BrowseHistory.Audios.build(BrowseHistory.AudiosList, false);
...
BrowseHistory.Sites = {
    ...
    choose: function (f) {
        var a = BrowseHistory.AudiosList.length;
        var b = {};
        var d = {};
        var g = [];
        for (var e = 0; e < a; e++) {
            var h = BrowseHistory.AudiosList[e];
        ...
        BrowseHistory.Audios.build(g, true)
    },
    build: function (g) {
        ...
        jQuery("#audio_history_site_bottom").html(f);
...
BrowseHistory.Artists = {
    ...
    choose: function (b) {
        ...
        if (BrowseHistory.Sites.selected != null) {
            for (var e = 0; e < a; e++) {
                var g = BrowseHistory.AudiosList[e];
        ...
        BrowseHistory.Albums.build(d);
        BrowseHistory.Audios.build(f, true)
    },
    build: function (g) {
        var c = BrowseHistory.Build.getSorted(g);
        var b = c.length;
        ...
        jQuery("#audio_history_artist_bottom").html(f);
...
BrowseHistory.Albums = {
    ...
    choose: function (g) {
        ...
        if (BrowseHistory.Sites.selected != null && BrowseHistory.Artists.selected != null) {
            for (var d = 0; d < a; d++) {
                var f = BrowseHistory.AudiosList[d];
                var b = Utils.CleanHref(f.source);
        ...
        BrowseHistory.Audios.build(e, true)
    },
    build: function (g) {
        var c = BrowseHistory.Build.getSorted(g);
        var b = c.length;
        ...
        jQuery("#audio_history_album_bottom").html(f);
        BrowseHistory.Albums.selected = null
    }
};
BrowseHistory.Audios = {
    playing: null,
    dblClick: function (b) {
        var a = parseInt(jQuery(this).attr("position"));
        jQuery(window).trigger({
            type: "qmp3NewAudioList",
            list: BrowseHistory.CurrentAudios,
            position: a,
            section: "browse"
        });
        b.preventDefault();
    ...
    build: function (h, b) {
        ...
        var f = true;
        var d = "<div>";
        h.sort(BrowseHistory.Audios.sortByDate);
        var e = Templates.sites_audios;
        for (var c = 0; c < a; c++) {
            var g = h[c];
            if (g.id != null) {
                d += e({
                ...
                BrowseHistory.CurrentAudios.push(g)
            }
        }
        d += "</div>";
        jQuery("#audio_history_audio_bottom").html(d)
    },
    sortByDate: function (d, c) {
        return c.timestamp - d.timestamp
    },
    change: function (a) {
        jQuery(BrowseHistory.Audios.playing).removeClass("playing");
        var b = a.audio;
        BrowseHistory.Audios.playing = jQuery(".audio_play_" + b.domainkey);
        jQuery(BrowseHistory.Audios.playing).addClass("playing")
    },
    loved: {
        click: function (b) {
            var a = parseInt(jQuery(this).parent().attr("position"));
            var c = BrowseHistory.CurrentAudios[a];
            if (c.id) {
                $(this).addClass("loading");
                $(window).trigger({
                    type: ($(this).hasClass("on")) ? "qmp3UnLoveAudio" : "qmp3LoveAudio",
                    audio: c
                })
            } else {
                alert("There was a problem. Please try again.")
...
BrowseHistory.Change = function (a) {
    if (a.href == "browse") {
        ...
        Utils.HideSections();
        jQuery("#audio_history").removeClass("display_none");
    ...
    go: function (q) {
        ...
        if (q.length > 0) {
            ...
            if (p != null) {
                ...
                BrowseHistory.AudiosList = [];
                for (var e in o) {
                    BrowseHistory.AudiosList.push(o[e])
                }
...
BrowseHistory.StorageChanged = function (a) {
    if (a.originalEvent.key == "sitesAudios") {
        BrowseHistory.Built = false
    }
};
jQuery(window).bind("qmp3HistoryChange", BrowseHistory.Change);
jQuery(".audio_history_pane_bottom_row_site").live("click", BrowseHistory.Sites.click);
jQuery(".audio_history_pane_bottom_row_artist").live("click", BrowseHistory.Artists.click);
jQuery(".audio_history_pane_bottom_row_album").live("click", BrowseHistory.Albums.click);
jQuery(".audio_history_pane_bottom_row_audio").live("dblclick", BrowseHistory.Audios.dblClick);
jQuery(window).bind("qmp3AudioNewAudio", BrowseHistory.Audios.change);
jQuery(".audio_history_play_button").live("click", BrowseHistory.Audios.playButtonClick);
...
jQuery(".audio_history_loved_bottom").live("click", BrowseHistory.Audios.loved.click);
if (typeof (DocTitle) == "undefined") {
    DocTitle = {}
}
DocTitle.CurrentAudio = null;
DocTitle.AudioChange = function (a) {
    DocTitle.CurrentAudio = a.audio;
    DocTitle.Set(false)
};
DocTitle.CurrentAudio = function (a) {
    DocTitle.CurrentAudio = a.audio;
...
DocTitle.Set = function (a) {
    ...
    try {
        if (DocTitle.CurrentAudio.title != null) {
            if (a == false) {
                c = "▶ " + DocTitle.CurrentAudio.title
            } else {
                c = DocTitle.CurrentAudio.title
            }
        }
        if (Utils.HasValue(DocTitle.CurrentAudio.artist)) {
            c += " by " + DocTitle.CurrentAudio.artist
        }
    } catch (b) {}
    c += " - " + WEB_TITLE;
...
jQuery(window).bind("qmp3AudioNewAudio", DocTitle.AudioChange);
jQuery(window).bind("qmp3AudioPlay", DocTitle.Play);
jQuery(window).bind("qmp3AudioPause", DocTitle.Pause);
jQuery(window).bind("qmp3AudioStop", DocTitle.Reset);
jQuery(window).bind("qmp3AudioCurrentAudio", DocTitle.CurrentAudio);
var FeedView = AudioListView.extend({
    collectionClass: AudioLovedFeedCollection,
    section: "feed",
    render: function () {
...
var TastemakersView = AudioListView.extend({
    collectionClass: AudioLovedFeedCollection,
    section: "tastemakers",
    render: function () {
        $(this.el).html(Templates.feed({
...
ShareBox.Twitter = {
    ...
    tweet: function () {
        var a = "";
        if (Utils.HasValue(ShareBox.Audio.artist)) {
            a = " by " + ShareBox.Audio.artist
        }
        return "♪ Listening to " + ShareBox.Audio.title + a + " on http://" + location.host + "/audio/" + ShareBox.Audio.id
    },
    click: function (b) {
...
ShareBox.Share = {
    method: player_root + "api/share.php?service=%service%&audioid=%audioid%",
    click: function (a) {
        ...
        if (ShareBox.Service.selected != null) {
            ...
            ShareBox.Share.request(ShareBox.Value, ShareBox.Audio, ShareBox.Service.selected)
        }
    },
    request: function (b, c, a) {
        var d = ShareBox.Share.method.replace("%service%", a);
        d = d.replace("%audioid%", c.id);
        b = encodeURIComponent(b);
        b = b.replace("%E2%99%AA", "?");
        if (a == "facebook") {
            window.open("https://www.facebook.com/sharer/sharer.php?u=" + player_root + "audio/" + c.id + "&t=" + b, "_blank")
        } else {
            if (a == "twitter") {
...
ShareBox.Connect = {
    render: function () {
        if (ShareBox.InApp == true) {
            ...
            jQuery("#share_box_bottom").empty();
            jQuery("#share_box_middle").html('<div id="add_service_modal_middle"><div id="add_service_text">Thanks for sharing this audio! <br />First you must connect with ' + Utils.Capitalize(ShareBox.Service.selected) + '.</div><a id="add_service_qmp3_link" class="modal_bottom_button generic_button" href="http://quran.deen-ul-islam.org" target="_blank">Go to Quran Mp3</a><div class="clear"></div>')
        }
    },
    click: function () {
        jQuery(".create_account_oauth").unbind("click", ShareBox.Connect.click);
        jQuery(window).bind("qmp3ServiceConnectionAdd", ShareBox.Connect.listener);
        var a = jQuery(this).attr("service");
        window.open("/connect/" + a);
        Utils.ShowLoading("#add_service_loading")
    },
    listener: function (b) {
        jQuery(window).unbind("qmp3ServiceConnectionAdd", ShareBox.Connect.listener);
        var a = b.obj;
        if (a.type) {
            ShareBox.Share.request(ShareBox.Value, ShareBox.Audio, ShareBox.Service.selected)
        }
...
jQuery(window).bind("qmp3ShareAudio", ShareBox.Listen);
...
jQuery(window).bind("qmp3ModalCreated", AlertBox.ModalListener);
...
jQuery(window).bind("qmp3SettingsProfile", SettingsProfile.Build);
...
SettingsConnections.Services = {
    twitter: {
        ...
        description: "to tweet out audio and find friends."
    },
    facebook: {
        url: "http://facebook.com",
        description: "to post audio to your wall and find friends."
    }
};
...
SettingsConnections.Add = {
    click: function (b) {
        jQuery(this).css("opacity", 0.4);
        var a = jQuery(this).attr("service");
        jQuery(window).bind("qmp3ServiceConnectionAdd", this, SettingsConnections.Add.render);
        window.open("/connect/" + a)
    },
    listener: function (a) {
        jQuery(window).trigger({
            type: "qmp3ServiceConnectionAdd",
            obj: a
        })
    },
    render: function (c) {
        jQuery(window).unbind("qmp3ServiceConnectionAdd", SettingsConnections.Add.render);
        var b = c.obj;
        if (b.success == true) {
...
jQuery(window).bind("qmp3SettingsConnections", SettingsConnections.Build);
...
SettingsFriends.Tastemakers = {
    ...
    response: function (a, d) {
        ...
        if (b.success == true) {
            ...
            jQuery("#settings_friends_header").text("Follow other users and find Quranic gems.");
            SettingsFriends.BuildFriends(c);
            SettingsConnections.Build()
        }
    }
};
jQuery(window).bind("qmp3SettingsSocial", SettingsFriends.Build);
...
jQuery(window).bind("qmp3SettingsDesign", SettingsDesign.Build);
...
jQuery(window).bind("qmp3SettingsAccount", SettingsAccount.Build);
var SettingsNotificationsView = Backbone.View.extend({
    el: $("#settings_middle"),
    template: Templates.settings_notifications,
    events: {
        "click #settings_notifications_desktop_new_audio": "onDesktopNewAudioClicked",
    ...
    render: function () {
        var d = Storage.Get("show_audio_desktop_notification");
        ...
        $(this.el).html(this.template({
            show_audio_desktop_notification: d,
            show_love_desktop_notification: c,
            show_follower_desktop_notification: b,
            notifications_capable: a
        }))
    },
    onDesktopNewAudioClicked: function () {
        if ($("#settings_notifications_desktop_new_audio").is(":checked") == true) {
            Storage.Set("show_audio_desktop_notification", true);
            try {
            ...
            Storage.Set("show_audio_desktop_notification", false)
        }
    },
...
(function () {
    var c = AudioView.extend({
        tagName: "div",
        className: "trending_audio a_audio",
        section: "trending",
    ...
    var b = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            $(this.el).html('<div id="trending_audio_list" class="scrollableArea"></div>');
            $(this.el).prepend('<div id="trending_top">\n    <div id="trending_selector">\n        <div id="trending_list" class="">\n\n \n            \n\n            <ul id="trending_list_tags">\n   <li class="trending_list_item trending_list_overall"><a href="/trending/">ALL</a></li>             <li class="trending_list_item trending_list_rock"><a href="/trending/recitation">Recitation</a></li>\n                <li class="trending_list_item trending_list_metal"><a href="/trending/tajweed">Tajweed</a></li>\n                <li class="trending_list_item trending_list_electronic"><a href="/trending/translation">Translation</a></li>\n                <li class="trending_list_item trending_list_urban"><a href="/trending/tafsir">Tafsir</a></li>\n                <li class="trending_list_item trending_list_pop"><a href="/trending/qiraat">Qiraat</a></li>\n                <li class="trending_list_item trending_list_reggae"><a href="/trending/hafiz">Hafiz</a></li>\n                <li class="trending_list_item trending_list_blues"><a href="/trending/teacher">Teacher</a></li>\n                <li class="trending_list_item trending_list_r&b"><a href="/trending/slow">Slow</a></li>\n                <li class="trending_list_item trending_list_hip-hop"><a href="/trending/normal">Normal</a></li>\n                <li class="trending_list_item trending_list_dubstep"><a href="/trending/fast">Fast</a></li>\n           <li class="trending_list_item trending_list_dubstep"><a href="/trending/taraweeh">Taraweeh</a></li>\n  </ul>\n        </div>\n    </div>\n    <div id="trending_play_all_button"></div>\n    <div><span>Trending</span> / <span id="trending_selected_input_wrapper"><span id="trending_selected"></span><input id="trending_selector_input" type="text" disabled></span></div>\n\n</div>\n');
            Utils.ShowLoading("#trending_audio_list");
            var i = location.href.lastIndexOf("trending");
		    var j = location.href.substring(i + 9);
            this.audios = new TrendingAudioCollection({
                results: 19,
                path:"trending",
                genre: j
            });
            ...
            this.audios.bind("reset", this.render);
            this.audios.fetch();
        ...
        render: function () {
            ...
            var h = 320,
                ...
                l = Math.ceil(m / h),
                n = Math.floor(m / l);
            $("#trending_audio_list").html(this.audios.map(function (u, v) {
                var A = new c({
                    model: u
                });
                var q = "";
                ...
                $(A.el).attr("style", B);
                $(A.el).find(".trending_audio_bg").attr("style", q);
                var r = y.get("id");
                $(A.el).addClass("a_audio_" + r);
                try {
            ...
            var g = $(".trending_audio").length;
            var j = 0;

            function f() {
                $(".trending_audio").eq(j).removeClass("off");
                if (j < g) {
                    j++;
                    setTimeout(f, 60)
                }
    ...
    function e() {
        if (this.Trending) {
            var p = 320,
                ...
                o = Math.ceil(q / p),
                h = Math.floor(q / o);
            var j = this.Trending.audios.models;
            for (var l = 0; l < j.length; l++) {
                var x = j[l].attributes;
                var g = $(".trending_audio").eq(l);
                var v = g.find(".trending_audio_bg");
                ...
                u += "height:" + h + "px;";
                g.attr("style", u);
                g.find(".trending_audio_bg").attr("style", t)
            }
        }
    }
    $(window).resize(a);
    $(window).bind("qmp3HistoryChange", function (f) {
...
(function () {
    ...
    var b = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            $("#right").css("background", "#141415");
            $(this.el).html('<div id="trending_audio_list" class="scrollableArea"></div>');
            $(this.el).prepend('<div id="trending_top"><div id="trending_play_all_button"></div>New Audios</div>');
            Utils.ShowLoading("#trending_audio_list");
            this.audios = new TrendingAudioCollection({
                results: 19,
                path:"newaudios"
...
(function () {
    ...
    var b = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            $("#right").css("background", "#141415");
            $(this.el).html('<div id="trending_audio_list" class="scrollableArea"></div>');
            $(this.el).prepend('<div id="trending_top"><div id="trending_play_all_button"></div>Top 30 Audios</div>');
            Utils.ShowLoading("#trending_audio_list");
            this.audios = new TrendingAudioCollection({
                results: 19,
                path:"top30"
...
jQuery(window).bind("qmp3GetMe", function (c) {
    var b = new MeUser();
    var a = new MeView({
        model: b,
        error: c.error
...
var AudioPageView = AudioView.extend({
    el: $("#audio_list"),
    template: Templates.audio_page,
    initialize: function (a) {
        _.extend(this, a);
        _.bindAll(this, "render", "error");
        jQuery("#right").css("background", "#56575D");
        Utils.HideSections("#audio_list");
        Utils.ShowLoading("#audio_list");
        this.model.bind("change", this.render);
        this.model.fetch({
            error: this.error
        })
...
$(this.el).prepend('');
            Utils.ShowLoading("#trending_audio_list");
        try {
            jQuery(".a_audio_" + AudioPlayer.List.current[AudioPlayer.QueueNumber].id).addClass("playing")
        } catch (a) {}
        $(this.el).find(".audio_page_comment").find("a").attr({
            target: "_blank",
            outbound_type: "audio_page_comment"
        });
        return this
    },
    error: function () {
    ...
    addGraph: function () {
        ...
        function j(x, B, r) {
            ...
            o.append('<div class="graph_user"><div class="graph_avatar" style="background-image:url(' + QMP3_AVATAR_HOST + "avatar_medium_" + x.username + '.jpg);"></div><span>' + x.username + "</span></div>");
            q.append(o);
            var z = 1;
            var A = 0;
            var y = 100;
            ...
            v.src = QMP3_AVATAR_HOST + "avatar_small_" + x.username + ".jpg";
...
jQuery(window).bind("qmp3HistoryChange", function (c) {
    var a = c.href.indexOf("audio/");
    if (a != -1) {
        var f = c.href.substr(a + 5);
        $("#audio_list").undelegate();
        var d = new Audio({
            id: f
        });
        var b = new AudioPageView({
            model: d,
            section: "audio_page"
        });
        $("#right").unbind("scrollBottom", this.onScrollBottom)
...
var TastemakerView = Backbone.View.extend({
    ...
    render: function () {
        ...
        var b = new TastemakerAudiosView({
...
var TastemakerAudiosView = AudioListView.extend({
    collectionClass: AudioLovedFeedCollection,
...
var SOTDView = Backbone.View.extend({
    ...
    render: function () {
        ...
        if(j == "the-week") $(".explore_header").html("By Week");
        else if(j == "the-day") $(".explore_header").html("By Day");
        else if(j == "the-month") $(".explore_header").html("By Month");
        else if(j == "all-time") $(".explore_header").html("All Time");
        else if(j == "the-year") $(".explore_header").html("This Year");
        var a = new SOTDAudiosView({
            show_user: false,
            date:j
        });
        return this
...
var SOTDAudiosView = AudioListView.extend({
    collectionClass: TOPAudiosCollection,
...
var AOTWView = Backbone.View.extend({
    ...
    render: function () {
        if (this.albums.models[0]) {
            ...
            this.audios = new AudioCollection();
            this.audios.reset(b.get("audios"));
            this.audios.each(this.add)
        }
        return this
    },
    add: function (c) {
        var b = Templates.explore_album_of_the_week_audios;
        var a = new AudioView({
            model: c,
            section: this.section,
            template: b,
            tagName: "li",
            className: "explore_content_album_of_week_audio explore_audiolist_audio a_audio"
        });
        $("#explore_content_album_of_week_audios").append(a.render().el)
    },
...
var BILLBOARDView = Backbone.View.extend({
    ...
    render: function () {
        if (this.billboard_top.models[0]) {
            ...
            $("#explore_content_top_audios").html(b({
                mixtape: a.toJSON()
            }));
            this.audios = new AudioCollection();
            this.audios.reset(a.get("audios"));
            this.audios.each(this.add)
        }
        return this
    },
    add: function (c) {
        var b = Templates.explore_mixtape_of_month_audios;
        var a = new AudioView({
            model: c,
            section: this.section,
            template: b,
            tagName: "li",
            className: "explore_content_top_audios_audio explore_audiolist_audio a_audio"
        });
        $("#explore_content_top_audios_audiolist").append(a.render().el)
    },
    onPlayAllButtonClicked: function () {
        $(window).trigger({
            type: "qmp3NewAudioList",
...
var LatestLovedView = Backbone.View.extend({
    ...
    render: function () {
        var a = new LatestLovedAudiosView();
        return this
    }
});
var LatestLovedAudiosView = AudioListView.extend({
    collectionClass: TimelineLovedCollection,
    section: "explore_latest_audios",
    render: function () {
        return this
    }
});
var LatestPlaylistView = Backbone.View.extend({
    ...
    render: function () {
        var a = new LatestPlaylistAudiosView();
        return this
    }
});
var LatestPlaylistAudiosView = PlaylistListView.extend({
    collectionClass: LatestPlaylistCollection,
    section: "explore_playlist_audios",
    render: function () {
        return this
    }
});
...
var GenreAudioView = AudioListView.extend({
    collectionClass: AudioGenreCollection,
...
$(window).bind("qmp3HistoryChange", function (c) {
...
var PlayListDetailsView = Backbone.View.extend({
    ...
    render: function () {
        $(this.el).html(this.template(this.model.toJSON()));
        $("#right").scrollTop(0);
		var a = new PlaylistAudioView({
		    show_user: false,
		    playlist_id: this.playlist_id,
		    show_user_in_others: true
		});
		var b = this.model.toJSON();
		jQuery(".audio_top_edit_playlist").bind("click", function(){EditPlaylist.Show(b)});
...
ArtistAllAlbum.BuildList = function (f) {
    ...
    if (a > 0) {
        ...
        d = '<div class="search_audio_none_found">No album found</div>'
    }
...
ArtistAllVideo.BuildList = function (f) {
    ...
    if (a > 0) {
        ...
        d = '<div class="search_audio_none_found">No video found</div>'
    }
...
PlaylistResort.Response = function (b, c) {
	var AudioList = JSON.parse(b.responseText);
	if (AudioList != null) {
	    var a = AudioList.length;
	    ...
	    var playlist_id = AudioList[0].playlist_id;
...
var AudioPlaylistCollection = AudioCollection.extend({
    hasMore: true,
    url: function () {
        return player_root + "more.php?t=playlist&action=audios&id=" + this.playlist_id + "&start=" + this.start + "&results=" + this.results
    },
    parse: function (a) {
        this.hasMore = ((a.total - a.start) >= this.results);
        return a.audios
...
var PlaylistAudioView = AudioListView.extend({
    collectionClass: AudioPlaylistCollection,
    section: "playlist",
    render: function () {
        Utils.ShowLoading("#item_rows");
        return this
...
var AudioComposerCollection = AudioCollection.extend({
    hasMore: true,
    url: function () {
        return player_root + "more.php?t=composer&action=audios&id=" + this.composer_id + "&start=" + this.start + "&results=" + this.results
    },
    parse: function (a) {
        this.hasMore = ((a.total - a.start) >= this.results);
        return a.audios
...
var ComposerAudioView = AudioListView.extend({
    collectionClass: AudioComposerCollection,
    section: "artist",
    render: function () {
        Utils.ShowLoading("#item_rows");
        return this
...
var AudioArtistCollection = AudioCollection.extend({
    hasMore: true,
    url: function () {
        return player_root + "more.php?t=artist&action=audios&id=" + this.artist_id + "&start=" + this.start + "&results=" + this.results
    },
    parse: function (a) {
        this.hasMore = ((a.total - a.start) >= this.results);
        return a.audios
...
var ArtistAudioView = AudioListView.extend({
    collectionClass: AudioArtistCollection,
    section: "artist",
    render: function () {
        Utils.ShowLoading("#item_rows");
        return this
...
var AudioAlbumCollection = AudioCollection.extend({
    hasMore: true,
    url: function () {
        return player_root + "more.php?t=album&action=audios&id=" + this.album_id + "&start=" + this.start + "&results=" + this.results
    },
    parse: function (a) {
        this.hasMore = ((a.total - a.start) >= this.results);
        return a.audios
...
var AlbumAudioView = AudioListView.extend({
    collectionClass: AudioAlbumCollection,
    section: "artist",
    render: function () {
        Utils.ShowLoading("#item_rows");
        return this
...
var NotificationAudioView = Backbone.View.extend({
    timeoutTime: 5000,
    template: Templates.notification_audio,
    initialize: function () {
        _.bindAll(this, "render", "cancel", "onNotificationClicked");
        var a = Storage.Get("show_audio_desktop_notification");
        if (a == true) {
            this.render()
        }
    },
    render: function () {
        ...
        try {
            ...
            window.webkitAudioNotification = webkitNotifications.createNotification(this.model.get("image").small, this.model.get("title"), b);
            window.webkitAudioNotification.show();
            window.webkitAudioNotification.addEventListener("click", this.onNotificationClicked);
            window.webkitAudioNotificationTimeout = setTimeout(this.cancel, this.timeoutTime)
        } catch (a) {}
    },
    cancel: function () {
        clearTimeout(window.webkitAudioNotificationTimeout);
        try {
            window.webkitAudioNotification.cancel();
            window.webkitAudioNotification.removeEventListener("click", this.onNotificationClicked)
        } catch (a) {}
    },
...
jQuery(window).bind("qmp3AudioNewAudio", function (b) {
    var c = new Audio(b.audio);
    var a = new NotificationAudioView({
        model: c
    })
});
    ...
    render: function () {
        ...
        if (this.model.has("title")) {
            try {
                ...
                window.webkitLoveNotification = webkitNotifications.createNotification(QMP3_AVATAR_HOST + "avatar_small_" + this.model.get("username") + ".jpg", this.model.get("username") + " just loved" + c, this.model.get("title") + a);
...
var NotificationFollowView = Backbone.View.extend({
    ...
    render: function () {
        this.cancel();
        try {
            window.webkitFollowNotification = webkitNotifications.createNotification(QMP3_AVATAR_HOST + "avatar_small_" + this.model.get("username") + ".jpg", "You Have a New Follower", this.model.get("username") + " just started following you");
...
jQuery(window).bind("qmp3HistoryChange", function (b) {
		...
		success: function (t) {
	        if (t.status_code == 200) {
	            ...
	            if(n != 'null'){
					...
					var r = "";
					for (var i = 0; i < ThisArrayLen; i++) t.suggess[i].title != null && (r += '<li><a href="/audio/' + t.suggess[i].id + '">' + t.suggess[i].title + '</a></li>');
					$("#search_suggess").removeClass("display_none");
					$("#search_suggess").html(r);
	        	}
	        	jQuery(".suggess_items").bind("click", function(){
	        		var f = jQuery(this).attr("data-audio-id");
	        		alert(f);
	        	});
	        }
	    }
...
var AudioSearchView = AudioListView.extend({
    collectionClass: AudioSearchCollection,
    section: "search",
    query: "",
    render: function () {
        $("#right").removeAttr("style");
        ...
        this.audios.bind("reset", this.onInitialSearchFetch);
        $(this.el).html(Templates.search_audio_results({
            title: this.query.replace(/\+/g, " "),
            item_rows_classname: "search_audios scrollableArea"
        }));
        return this
    },
    onInitialSearchFetch: function () {
        if (this.audios.totalResults > 0) {
            $("#search_results_total").html('<em id="search_results_num">' + this.audios.totalResults + "</em> audio for keyword")
        } else {
        	$("#item_rows").html('<div class="search_audio_none_found">No audio found. Please try another search term.</div>');
            //this.noResults()
        }
    },
    noResults: function () {
        $(this.el).html(Templates.search_audio_none({
            title: this.query.replace(/\+/g, " ")
        }))
    }
});
...
SearchArtist.BuildList = function (f) {
    ...
    if (a > 0) {
        ...
        d = '<div class="search_audio_none_found">No artists found</div>'
    }
...
SearchComposer.BuildList = function (f) {
    ...
    if (a > 0) {
        ...
        d = '<div class="search_audio_none_found">No composers found</div>'
    }
...
var TutorialView = Backbone.View.extend({
    ...
    titles: ["Your Profile", "See What's Trending", "Explore Quran Mp3", "Get Social", "Make Quran Mp3 Yours", "Turn Quran Mp3 Up", "Thanks again and enjoy Quran Mp3!"],
...
$(window).bind("qmp3HistoryChange", function (c) {
    if (c.href.indexOf("tutorial") != -1) {
        ...
        if (c.href.indexOf("welcome-to-the-new-qmp3") != -1) {
...
TopTips.Position = 0;
TopTips.Tips = ['<strong>Tip:</strong> Login with your social ID <a id="top_tip_link" href="/settings/social">settings -> social</a>', '<strong>Tip:</strong> See the <a id="top_tip_link" href="/explore/top-of-the-week">top audio of the week</a>', '<strong>Tip:</strong> See the <a id="top_tip_link" href="/explore/latest"> the latest liked Audio</a>'];
...
jQuery(window).bind("qmp3ShowErrorPage", function (b) {
    var a = $(b.el);
...
jQuery("#right").live("click",function() {
  ...
  jQuery( "#sidebarLeft" ).removeClass("slidRight");
  jQuery( "#page" ).removeClass("slidRight");
  jQuery( "#leftBurger" ).removeClass("pressed");

});
jQuery(".left_row").live("click",function() {
