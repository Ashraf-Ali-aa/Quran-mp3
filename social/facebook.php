...
if( $oauth_token ) {
	...
	if($logged){
		...
		profile_image_url = 'https://graph.facebook.com/" . $user->id . "/picture?type=large', token = '" . $params['access_token'] . "', `date` = '$_TIME'
		");
		...
		if( $row['user_id'] ) {
			...
			if( $member_id['user_id'] ){
				...
				$buffer ['image'] = avatar ( $member_id ['avatar'], $member_id ['user_id'] );
