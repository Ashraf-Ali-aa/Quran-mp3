...
if (!@is_dir ( ROOT_DIR . '/static/audios/')) {
	die ( "Audios not found!" );
}

if(!is_writable(ROOT_DIR . '/static/audios/')) {
	
	$fail_upload = '<div class="alert alert-error">
				<button data-dismiss="alert" class="close" type="button">×</button>
				<strong>Warring!</strong> folder <strong>/static/audios/</strong> is not writable!.
			</div>';
...
if(isset($_POST['submit'])){
	...
	for($i=0;$i<count($titles);$i++){
		...
		if($titles[$i] && $artist[$i]){
			...
			if($artist_id['id']){
				...
				$db->query("INSERT INTO  vass_audios (title, artist_id, album_id, user_id, active, created_on) VALUES ('" . $db->safesql($titles[$i]) . "', '" . $artist_id['id'] . "', '" . $album_id['id'] . "', '" . $member_id['user_id'] . "', '1', '$_TIME')");
				
				$audio_id = $db->insert_id();
				
				@rename(ROOT_DIR . "/static/audios/temp/" . $filename[$i], ROOT_DIR . "/static/audios/" . $audio_id . ".mp3");
	...
	msg_page("success", "<strong>Well done!</strong> Saved audio!", "do=upload");
...
%}
    <tr class="template-download fade">
        ...
        {% } else { %}
            <td class="name">
                 <input type="text" autocomplete="off" placeholder="Audio name" id="title" required="" name="title[]" value="{%= file.name.replace('.mp3','') %}" style="margin-bottom:0;">
				<input type="hidden" name="filename[]" value="{%=file.name%}">
                <!--<a href="{%=file.url%}" title="{%=file.name%}" data-gallery="{%=file.thumbnail_url&&'gallery'%}" download="{%=file.name%}">{%=file.name%}</a>-->
            </td>
            <td class="album">
                 <input type="text" autocomplete="off" placeholder="Album of the audio" id="album" name="album[]" class="auto_album" style="margin-bottom:0;" onclick="auto_album({%= rand_number %}); return false;">
            </td>
            <td class="artist">
                 <input type="text" autocomplete="off" placeholder="Artist who own the audio" id="artist" required="" name="artist[]" class="auto_artist" style="margin-bottom:0;" onclick="auto_artist({%= rand_number %}); return false;">
