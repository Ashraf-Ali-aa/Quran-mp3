...
$thirst_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[thirst_source]", "{$config['thirst_source']}" );
$request_type = makeDropDown( array ("1" => 'Curl', "2" => 'File Get Contents' ), "save_con[content_type]", "{$config['content_type']}" );
$wuzam_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[wuzam_source]", "{$config['wuzam_source']}" );
$dilandau = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[dilandau]", "{$config['dilandau']}" );
$degjo_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[degjo_source]", "{$config['degjo_source']}" );
$emp3world_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[emp3world_source]", "{$config['emp3world_source']}" );
$mp3sru_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[mp3sru_source]", "{$config['mp3sru_source']}" );
$listengo_source = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[listengo_source]", "{$config['listengo_source']}" );
$dilandau = makeDropDown( array ("1" => 'On', "0" => 'Off' ), "save_con[dilandau]", "{$config['dilandau']}" );
			...
			<div class="tab-content" id="myTabContent">
				...
				<div id="seo" class="tab-pane fade">
					<fieldset>
						...
						<input class="input-xxlarge" type="text" name="save_con[latest_love_page]" value="{$config[latest_love_page]}" placeholder="Latest loved audios page option title"/>
						...
						<h3>Audio page option</h3>
						<hr>
						
						<label>Audio page title</label>
						<input class="input-xxlarge" type="text" name="save_con[audio_page_title]" value="{$config[audio_page_title]}" placeholder="User %name% for audio title, %artist% and %sitetitle% for webtitle"/>
						<label>Audio page description</label>
						<textarea name="save_con[audio_page_descr]" style="width: 530px; height: 100px">{$config[audio_page_descr]}</textarea>
						
						<h3>Search page option</h3>
						<hr>
						
						<label>Search page title</label>
						<input class="input-xxlarge" type="text" name="save_con[search_page_title]" value="{$config[search_page_title]}" placeholder="User %name% for audio title, %artist% and %sitetitle% for webtitle"/>
