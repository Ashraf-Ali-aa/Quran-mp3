...
if( $action == "edit" || $action == "add" ){
	...
	if($action == "edit"){
		...
		if(isset($_POST['name'])){
			...
			$db->query("UPDATE vass_albums SET tag = '$tags_id', name = '$name', artist_id = '$artist_id', descr = '$descr', active = '$active' WHERE id = '$id'");
		...
		$row = $db->super_query("SELECT vass_albums.name, vass_albums.active, vass_albums.id, vass_artists.id AS artist_id, vass_artists.name AS artist FROM vass_albums LEFT JOIN vass_artists
	ON vass_albums.artist_id = vass_artists.id WHERE vass_albums.id = '$id'");
		...
		if(isset($_POST['name'])){
			...
			$db->query("INSERT INTO vass_albums SET tag = '$tags_id', name = '$name', artist_id = '$artist_id', descr = '$descr', active = '$active'");
...
HTML;
	...
	echo <<<HTML
			...
			<p style="float:right;20px;">
				<button onClick="document.location='?do=albums&action=add'; return(false)" id="upload_audio_button" class="btn btn-success" type="button"><i class="icon-plus icon-white"></i>Add new album</button>
			</p>
