...
if( $action == "edit" ){
	...
	if(isset($_POST)){
		...
		if($title){
			$db->query("UPDATE vass_lyrics SET title='$title', artist='$artist', lyrics='$lyrics' WHERE id='$id'");
			define ( 'SUBMIT', true );
		}
	}
	
	$row = $db->super_query("SELECT * FROM vass_lyrics WHERE id = '$id'");
...
HTML;
	...
	if($id){
			$db->query("DELETE FROM vass_lyrics WHERE id='$id'");
			define ( 'DELETE', true );
...
HTML;
	...
	$start = ($page-1) * 50;
	$db->query("SELECT * FROM vass_lyrics LIMIT $start,20");
...
HTML;
	...
	$total = $db->super_query("SELECT COUNT(*) AS count FROM vass_lyrics");
