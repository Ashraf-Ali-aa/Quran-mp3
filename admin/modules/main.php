...
if(is_writable(ROOT_DIR . "/static/audios/")){
	$static_audios_status = '<p class="text-success">/static/audios/</strong> is writable.</p>';
}else{
	$static_audios_status = '<p class="text-error">/static/audios/</strong> is not writable please CHMOD this folder to 777.</p>';
}
...
$row = $db->super_query("SELECT COUNT(*) AS count FROM vass_audios");
$total_audio = $row['count'];

$row = $db->super_query("SELECT COUNT(*) AS count FROM vass_users");
$total_user = $row['count'];

$row = $db->super_query("SELECT COUNT(*) AS count FROM vass_audio_love");
				...
				<div id="global" class="tab-pane fade active in">
					...
					<table class="table">
						...
						<tbody>
							...
							<tr>
								<td>Audio store folder writeable</td>
								<td>{$static_audios_status}</td>
							</tr>
						</tbody>
					</table>
				</div>
